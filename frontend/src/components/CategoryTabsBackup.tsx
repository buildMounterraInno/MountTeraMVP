...
export default function CategoryTabs() {
  ...
  return (
    <div className="max-h-screen w-full bg-white">
      {/* Tabs Container */}
      <div className="flex -translate-y-13 justify-around">
          {categories.map((category) => (
            <button
              key={category.id}
              className={`font-tpc px-6 pt-2 pb-3 rounded-t-lg text-2xl font-medium transition-colors focus:outline-none ${
